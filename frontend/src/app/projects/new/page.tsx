...
export default function NewProject() {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Convert to lowercase, remove special characters, replace spaces with dashes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Failed to create project. Please verify if slug is unique.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-6 py-10 flex flex-col gap-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-400 text-sm">
            Initialize a new application space, specify its target stack, and build integrations.
          </p>
        </div>
        ...
        <section className="glass-panel rounded-2xl p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Slug */}
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 block">Slug * (Auto-generated)</label>
                <input
                  type="text"
                  placeholder="e.g. job-pilot-ai"
                  value={slug}
